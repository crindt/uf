...
from footprint.utils.subclasses import match_subclasses
...
class FeatureResource(DynamicResource):
    ...
    def create_subclass(self, params, config_entity=None, db_entity_key=None, **kwargs):
        ...
        feature_resource_instance = self.resolve_feature_resource_instance(feature_class)
        return feature_resource_instance.dynamic_resource_class(params, feature_class)

    def resolve_feature_resource_instance(self, feature_class):
        # If not already subclassed
        if self.__class__ == FeatureResource:
            # We assume that only one FeatureResource subclass has a _meta.queryset.model that is a parent class of the given feature_class
            feature_resource_class = match_subclasses(self.__class__, lambda resource_class: issubclass(feature_class, resource_class._meta.queryset.model))[0]
            return feature_resource_class()
        return self
