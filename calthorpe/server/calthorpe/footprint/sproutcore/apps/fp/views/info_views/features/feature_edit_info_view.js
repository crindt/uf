...
sc_require('views/client/scag/info_views/scag_general_plan_parcel_feature_info_view');
...
Footprint.FeatureEditInfoView = SC.View.extend({
    ...
    contentView: Footprint.ContentView.extend({
        ...
        */
            ...
            nowShowingBinding:SC.Binding.oneWay('.recordType').transform(function(recordType) {
                if (recordType)
                    return '%@InfoView'.fmt(recordType.toString().split('.')[1].camelize());
                return this.get('nowShowingDefault')
            }),

            defaultEditView:SC.View,

            scagPrimaryParcelFeatureInfoView:Footprint.ScagPrimaryParcelFeatureInfoView.extend({
                contentBinding: SC.Binding.oneWay('.parentView.content')
            }),

            scagGeneralPlanParcelFeatureInfoView:Footprint.ScagGeneralPlanParcelFeatureInfoView.extend({
                contentBinding: SC.Binding.oneWay('.parentView.content')
            }),

            scagPrimarySpzFeatureInfoView:Footprint.ScagPrimarySpzFeatureInfoView.extend({
                contentBinding: SC.Binding.oneWay('.parentView.content')
            })
        }),
