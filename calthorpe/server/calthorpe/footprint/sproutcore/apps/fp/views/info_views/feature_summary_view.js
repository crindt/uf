...
Footprint.FeatureSummaryView = Footprint.InfoView.extend({
    ...
    contentView: SCTable.TableView.design({
        ...
        content:function() {
            // Make sure the status of the content is READY_CLEAN before loading. It will never be READY_DIRTY because
            // this content is from the main store, which is not edited directly.
            if (!this.get('recordType') || !this.get('_content') || !(this.get('_status') === SC.Record.READY_CLEAN))
                return [];
